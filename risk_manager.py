...
def clear_files(time_now):
    #list files, grab first letter as ts
    files = [f for f in os.listdir('.') if os.path.isfile(f)]
    for f in files:
        filename = f.replace('_', ':')
        if not re.search(regex, filename):
            continue
        matched = re.search(regex, filename).group()
        if matched:
            file_ts = datetime.fromisoformat(matched)
            if time_now - file_ts > dt.timedelta(hours=1):
                os.remove(f)
...
class RiskManager:
    ...
    def to_csv(self):
        ...
        clear_files(time_now)
        ts = time_now.isoformat().replace(':', '_')
        path = '{}-{}'.format(ts, id)
        self.df.to_csv('{}.csv'.format(path))
        return path
